...
def login():
    ...
    if user:
        if sha256_crypt.verify(password, user.hased_password):
            ...
            return jsonify('first'), 401
    else:
        return jsonify('second'), 401
...
@app.route('/asset/by-user')
@jwt_required()
def home():
    user_assets = Asset.query.filter_by(owner_id=get_jwt_identity()).all()
    list = []
    for asset in user_assets:
        list.append([asset.id, asset.name, asset.description, asset.price, asset.owner_id])
    return jsonify(list), 200

@app.route('/asset/by-user/<int:asset_id>')
@jwt_required()
...
@jwt_required()
def top():
    user_assets = Asset.query.filter_by(owner_id=get_jwt_identity()).order_by(Asset.price.desc()).limit(5).all()
    list=[]
    for asset in user_assets:
        list.append([asset.id,asset.name,asset.description,asset.price,asset.owner_id])
    return jsonify(list),200
...
def add_asset():
    ...
    update_top_assets_cache(get_jwt_identity())

    if len(name) < 1:
        return jsonify('Bad name'), 401
    else:
        new_asset = Asset(name=name, description=description, price=price, owner_id=get_jwt_identity())
...
def delete_asset(asset_id):
    ...
    if asset and asset.owner_id == get_jwt_identity():
...
def buy_asset(asset_id):
    ...
    if Asset.query.get(asset_id) is None or User.query.filter_by(id=owner_id) is None:
